...
class Rest {
	...
	public function update_settings( WP_REST_Request $request ) {
		try {
            ...
            try {
                if ( !$this->api->check_api_key_validity($options['openai']['apikey']) ) {
                    $plugin_option['openai']['apikey'] = '';
                }

                if ( !$this->googleAi->check_api_key_validity($options['googleai']['apikey']) ) {
                    $plugin_option['googleai']['apikey'] = '';
                }

                if ( !$this->claude->check_api_key_validity($options['claude']['apikey']) ) {
                    $plugin_option['claude']['apikey'] = '';
                }

                if ( !$this->openRouter->check_api_key_validity($options['openrouter']['apikey']) ) {
