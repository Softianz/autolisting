...
function submit_meeting_schedule()
{
	...
	if ($ok) {
		...
		$email = get_post_meta($post->ID, '_job_email', true);

		$message = "Hello , You have new meeting for listing $postTitle at $dateTime with $userName .";
		wp_mail($email, 'New Meeting Schedule', $message);

		$message1 = "Hello, You have schedule  with $authorName from listing $postTitle at $dateTime";
		wp_mail($params['user_email'], 'New Meeting Schedule', $message1);
...
function change_meeting_status_callback()
{
	...
	// Send mail buyer
	$phone = get_post_meta($list->ID, '_job_phone', true);
	$email = get_post_meta($list->ID, '_job_email', true);

	if ($status == 'Accept') {

		$message = " Hello $userName, Your meeting for listing $postTitle is $status by Seller. You can contact him with email : $email and Phone number : $phone";
		$message1 = " Hello, You have $status meeting for listing $postTitle with seller Name: $userName, Buyer email : $mettings->user_email and Buyer Phone : $mettings->user_phone";
	} else {
		$message = " Hello $userName, Your meeting for listing $postTitle is $status by Seller.";
		$message1 = " Hello, You have $status meeting for listing $postTitle with $userName.";
	}
	wp_mail($mettings->user_email, 'Meeting Schedule Status', $message);
	wp_mail($email, 'Meeting Schedule Status', $message1);
...
function listing_sold_out_callback()
{
	...
	if ($results) {
		foreach ($results as $result) {
			...
			wp_mail($result->user_email, 'Listing is sold out', $message);
		}
	}
...
function send_mail_5_min_before_metting_callback()
{
	...
	if ($meetings) {
		foreach ($meetings as $meeting) {
			...
			$userName = $user->display_name;
			$AuthorName = $author->display_name;
			$email = get_post_meta($meeting->ID, '_job_email', true);
			...
			wp_mail($meeting->user_email, 'Meeting Schedule Status', $message);
			wp_mail($email, 'Meeting Schedule Status', $message1);
		}
	}
}
...
add_action('wp_ajax_my_action', 'handle_paypal_approval_callback');
add_action('wp_ajax_nopriv_my_action', 'handle_paypal_approval_callback');
function handle_paypal_approval_callback() {
    check_ajax_referer('ajax-nonce', 'nonce');

    // Your AJAX action handling code here
